...
`POST /v0/profile/normalize/preview` 接收仍在生命周期内的 F002 任务，使用本地规则规范化日期、联系方式、数字和常见字段，并生成带来源、证据、置信度、问题和冲突状态的待确认候选。教育、工作、实习和可识别的项目片段按来源位置生成重复记录建议；无法可靠归类的内容保留人工处理状态，首版不自动合并已有记录。`/confirm` 只接受用户明确确认的字段或记录，并复用 F001 的资料版本校验和原子写入；`/cancel` 会使任务失效。F003 默认不调用远程模型，不上传原文、证据或候选。
