...
class FakeWriter:
    ...
    def write_atomic(self, destination, payload):
...
def test_representative_fakes_satisfy_protocols_without_io():
    ...
    assert isinstance(keys, KeyProvider)
    assert isinstance(writer, AtomicWriter)
    assert store.read() is None
    marker = object.__new__(ProfileSnapshot)
    store.write(marker)
    assert store.read() is marker
    store.delete()
    assert store.deleted
    assert keys.get_key() is None
    assert keys.provision_key() == b"synthetic"
    assert keys.destroy_key()
    destination = Path("synthetic-profile.bin")
    writer.write_atomic(destination, b"ciphertext")
    assert writer.calls == [(destination, b"ciphertext")]
